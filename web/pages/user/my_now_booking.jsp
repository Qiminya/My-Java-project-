...
<div id="table">
    <div class="table_mark" style="background-color: #e3e3e3">我当前预约的图书</div>
    <c:forEach items="${requestScope.bookings}" var="booking">
        <div class="book_div1">
            <div class="book_div2"><img class="img" src="${booking.book_imgpath}"></div>
            <div style="margin: 0px; background-color: #e3e3e3">
                <span><a href="bookServlet?action=getBookById&id=${booking.book_id}">${booking.book_name}</a></span><br>
                <span style="font-size: 15px;">${booking.booking_time}</span>
            </div>
        </div>
    </c:forEach>
</div>
